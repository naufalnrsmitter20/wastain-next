...
export const SecondaryButton = ({ children, className, type, onClick, target }: IButton) => {
  return (
    <button
      className={`text-[14px] text-primary-green border transition-all duration-200 border-primary-green hover:bg-dark-green hover:text-white font-bold uppercase py-[8px] rounded-[10px] ${className}`}
      onClick={onClick}
      type={type}
      formTarget={target}
    >
