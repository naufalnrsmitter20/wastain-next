...
export default function ProductDetail({ data, params }: { data: Prisma.ProductsGetPayload<{}>[]; params: { slug: string } }) {
  ...
  return (
    <>
      <main className="max-w-6xl flex justify-start mt-28 mx-auto">
        <div className="mb-10 w-full h-full max-w-xl">
          <Image src={product.image} alt={product.nama_barang} width={100} height={100} className="rounded-[10px] object-cover w-full h-full max-w-[536px]" />
        </div>
        <div className="ml-12">
          <h4 className="font-bold text-[24px] tracking-wide">{product.nama_barang}</h4>
          <div className="flex">
          ...
          <div className="flex justify-start gap-x-6 items-center">
            <div>
              <p className="font-medium text-[14px] text-gray-3 leading-loose">Kategori</p>
              <p className="font-medium text-[14px] text-gray-3 leading-loose">Tipe</p>
              <p className="font-medium text-[14px] text-gray-3 leading-loose">Stok</p>
              <p className="font-medium text-[14px] text-gray-3 leading-loose">Status</p>
            </div>
            <div>
              <p className="font-medium text-[16px] leading-loose">: {product.kategori}</p>
              <p className="font-medium text-[16px] leading-loose">: {product.tipe}</p>
              <p className="font-medium text-[16px] leading-loose">: {product.stok}</p>
              <p className="font-medium text-[16px] leading-loose">: {product.stok > 0 ? "Stok Tersedia" : "Stok Habis"}</p>
            </div>
          </div>
          <div>
            <h5 className="font-bold mt-3 text-[16px] text-gray-3 leading-loose">Detail Produk</h5>
          ...
          {status === "unauthenticated" ? (
            ...
            <>
              {product.stok !== 0 && (
                <div>
                  <AddToCartActions data={data} params={params} />
                </div>
              )}
            </>
          )}
