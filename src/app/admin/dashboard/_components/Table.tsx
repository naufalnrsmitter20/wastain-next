...
export default function AdminTable({ customer, product, order, admin }: { customer: Prisma.UserGetPayload<{}>[]; product: Prisma.ProductsGetPayload<{}>[]; order: Prisma.OrderGetPayload<{}>[]; admin: Prisma.UserGetPayload<{}>[] }) {
  ...
  const [filteredStudent, setFilteredStudent] = useState<Prisma.UserGetPayload<{}>[]>(admin);
  const router = useRouter();

  useEffect(() => {
    const filterProjects = () => {
      const filteredByName = admin.filter((x) => x.username.toLowerCase().includes(searchInput.toLowerCase()));
  ...
  return (
    <>
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-start gap-6 w-full max-w-screen-2xl">
        <div className="flex flex-col text-center xl:text-start gap-4 shadow-sm rounded-lg p-4 shadow-primary-green">
          <div>
            <h4 className="font-normal text-2xl text-black">Total Customer</h4>
            <h3 className="mb-2 text-4xl font-bold tracking-tight text-primary-green">{customer.length}</h3>
          </div>
          <p className="text-secondary-text-color">Total customer yang pada wastain</p>
        </div>
        <div className="flex flex-col text-center xl:text-start gap-4 shadow-sm rounded-lg p-4 shadow-primary-green">
          <div>
            <h4 className="font-normal text-2xl text-black">Total Product</h4>
            <h3 className="mb-2 text-4xl font-bold tracking-tight text-primary-green">{product.length}</h3>
          </div>
          <p className="text-secondary-text-color">Total product yang pada wastain</p>
        </div>
        <div className="flex flex-col text-center xl:text-start gap-4 shadow-sm rounded-lg p-4 shadow-primary-green">
          <div>
            <h4 className="font-normal text-2xl text-black">Total Order</h4>
            <h3 className="mb-2 text-4xl font-bold tracking-tight text-primary-green">{order.length}</h3>
          </div>
          <p className="text-secondary-text-color">Total order yang pada wastain</p>
        </div>
      </section>
      <>
        <div className="relative">
          <div className="flex gap-4 items-center mb-4 my-4">
            ...
            <button
              type="submit"
              className="focus:outline-none text-white bg-primary-green peer-focus:bg-green-700 hover:bg-green-700 focus:ring-2 focus:ring-green-300 font-medium text-sm px-5 py-2.5 me-2 flex w-fit items-center rounded-md"
            >
