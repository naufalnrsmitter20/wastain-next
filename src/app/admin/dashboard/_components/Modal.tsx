...
export default function Modal({ setIsOpenModal, data }: { setIsOpenModal: Dispatch<SetStateAction<boolean>>; data: Prisma.UserGetPayload<{}> }): React.JSX.Element {
  ...
  const HandleSubmit = async (e: ChangeEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // await UpdateUserByIdInAdmin(data?.user_id as string, formData);
  ...
  return (
    <ModalProfile title="Data Student" onClose={() => setIsOpenModal(false)}>
      <form onSubmit={HandleSubmit}>
        <TextField required type="text" label="Name" name="name" defaultValue={data?.username as string} placeholder="Nama Lengkap" />
        ...
        <div className="flex justify-end w-full gap-x-4 pb-4 mt-4">
          <SecondaryButton type="button" onClick={() => setIsOpenModal(false)}>
            Close
          </SecondaryButton>
